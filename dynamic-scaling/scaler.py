...
T = 0.1 # segons que te de mitja tractar peticions (S'ha augmentat ja que hauria de ser 1/C però si no no escalaria)
...
canal.queue_declare(queue='insultChannel')
canal.queue_bind(exchange='insultExchange', queue='insultChannel')

canal.queue_declare(queue='countChannel')
canal.queue_bind(exchange='insultExchange', queue='countChannel')

canal.queue_purge(queue='insultChannel')
canal.queue_purge(queue='countChannel')
