...
def on_new_message(body, props):
    msg = body_to_process.decode()
    if msg == "1":
        print("Sending result list...")
        list_result = ", ".join(result)
        channel.basic_publish(exchange='', routing_key=props.reply_to,body=list_result)
    else:
        global response
        response = None  
        channel.basic_publish(exchange='',routing_key='insultChannel1',properties=pika.BasicProperties(reply_to=callback_queue),body='3')
...
channel.queue_declare(queue='WorkQueue1')
result_queue = channel.queue_declare(queue='', exclusive=True)
callback_queue = result_queue.method.queue

print("Waiting for messages...")

for method_frame, properties, body_to_process in channel.consume('WorkQueue1', inactivity_timeout=1):
    if method_frame:
        on_new_message(body_to_process, properties)
