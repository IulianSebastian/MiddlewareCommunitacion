...
def on_new_message(body, props):
    ...
    if msg == "1":
        ...
        global response
        response = None  
        channel.basic_publish(exchange='',routing_key='insultChannel3',properties=pika.BasicProperties(reply_to=callback_queue),body='3')
...
channel.queue_declare(queue='WorkQueue3')
result_queue = channel.queue_declare(queue='', exclusive=True)
callback_queue = result_queue.method.queue

print("Waiting for messages...")

for method_frame, properties, body_to_process in channel.consume('WorkQueue3', inactivity_timeout=1):
    if method_frame:
